...
void ListaSimple::insertarInicio(string _dato){
	if(isEmpty()){
...
void ListaSimple::insertarAlFinal2(string dato){
	if(isEmpty()){
...
bool ListaSimple::estaAhi(string _dato){
	Nodo *temp=primerNodo;
	while(temp!=NULL){
		if(temp->dato==_dato){
			return true;
...
int ListaSimple::apariciones(string _dato){
	int cantidad=0;
	if(largoLista()>0){
		Nodo *temp=primerNodo;
		while(temp!=NULL){
...
void ListaSimple::insertarPos(int pos, string _dato){
